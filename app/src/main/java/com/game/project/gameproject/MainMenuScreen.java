...
import com.game.project.gameproject.MVC.GameScreen;
...
public class MainMenuScreen extends Screen {
    public MainMenuScreen(Game mrNomGame) {
        super(mrNomGame);
    }
        ...
        for(int i = 0; i < len; i++) {
            ...
            if(event.type == Input.TouchEvent.TOUCH_UP) {
                ...
                if(inBounds(event, 0, 0, 50,50) ) {
                    LoadingScreen scr;
                    if(((AndroidGame) game).styleType == LoadingScreen.TYPE_NORMAL){
                        ((AndroidGame) game).styleType = LoadingScreen.TYPE_CUSTOM;
                        scr = new LoadingScreen(game);
                    }else{
                        ((AndroidGame) game).styleType = LoadingScreen.TYPE_NORMAL;
                        scr = new LoadingScreen(game);
                    }
                    game.setScreen(scr);
                    if(Settings.soundEnabled)
